...
class SimulationBase(BaseModel):
    ...
    blade_files: list[str] = Field(
        default_factory=list,
        description="Paths to blade input files used by the simulation",
    )
    tower_file: Optional[str] = Field(
        None,
        description="Path to the tower input file",
    )
    controller_file: Optional[str] = Field(
        None,
        description="Path to the controller or DLL file",
    )
    simulation_type: Optional[str] = Field(
        None,
        max_length=100,
        description="Simulation type (e.g., DLC, power curve)",
    )
    iec_dlcs: list[str] = Field(
        default_factory=list,
        description="IEC DLC identifiers covered by this simulation",
    )
...
class SimulationUpdate(BaseModel):
    ...
    blade_files: Optional[list[str]] = Field(
        None,
        description="Paths to blade input files used by the simulation",
    )
    tower_file: Optional[str] = Field(
    ...
    iec_dlcs: Optional[list[str]] = Field(
        None,
        description="IEC DLC identifiers covered by this simulation",
    )
